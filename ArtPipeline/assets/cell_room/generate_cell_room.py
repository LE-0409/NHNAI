...
치수: 내부 8.0 × 8.0 × 6.0 m, 벽 두께 0.20 m. 원점은 방 바닥 중앙(Z=0).
치수를 바꾸면 `generate_ceiling_lamp.py` 의 H 와 `CellRoomBootstrap.cs` 의 상수도 같이 고친다.
"""
import os
import sys
...
# --- 치수 --------------------------------------------------------------------
# 높이를 키우는 건 방을 넓히기 위해서만이 아니다. 전등이 높아야 빛 기둥이 길어지고,
# 길어야 고깔 형태가 읽힌다. 천장이 낮으면 빛이 퍼지기도 전에 바닥에 닿는다.
W = 8.0        # 내부 폭 (X)
D = 8.0        # 내부 깊이 (Y)
H = 6.0        # 내부 높이 (Z)
T = 0.20       # 벽 두께
...
HW, HD = W / 2, D / 2
SKIRT_H = 0.22   # 걸레받이 높이
SKIRT_T = 0.05   # 걸레받이가 벽에서 튀어나온 두께
