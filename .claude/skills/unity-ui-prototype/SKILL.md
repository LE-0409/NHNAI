...
description: >
  ...
  개발 파이프라인 1단계(프로토타입 작성)를 담당하며, USS 호환 CSS 규칙을 지킨
  HTML/CSS를 prototype/ 에 생성한다. 기존 컴포넌트의 색·간격 조정이나 라벨 문구 교체처럼
  프로토타입을 건너뛰는 작업(unity-ui-authoring 스킬 참조)에는 사용하지 않는다.
---

# unity-ui-prototype — 화면 프로토타입 제작

새 화면을 Unity에 만들기 전에 브라우저에서 레이아웃을 확정하는 HTML/CSS를 만든다.
이 스킬은 **`unity-ui-authoring` 스킬 「개발 파이프라인」의 1단계**다. 결과물은 `prototype/`에 두고,
...
이 프로젝트는 전역 토큰 파일을 두지 않는다 (`unity-ui-authoring` 스킬 「디자인 값은
화면마다 로컬 변수로 모은다」).
...
- 클래스는 **BEM** (`block__element--modifier`). 규칙은 `unity-ui-authoring` 스킬 ·
...
- **화면에 나가는 문구는 영어로 쓴다.** 기본 폰트에 한글 글리프가 없어 빌드에서만
  빈칸이 된다 (`unity-ui-authoring` 스킬 「폰트 준비」).
...
이후 변환(2단계~)은 `unity-ui-authoring` 스킬 「개발 파이프라인」 절차를 따른다.
